...
from morris_pratt_pattern_search import morris_pratt_pattern_search as mp
...
for i in x_axis_p:
    ...
    y_axis_mp_p.append(mp(pattern_p, text_p)[1])
...
for i in x_axis_a:
    ...
    y_axis_mp_a.append(mp(pattern_a, text_a)[1])
...
plt.plot(x_axis_t, y_axis_mp_t, label='Morris-Pratt Pattern Search O(T)')
plt.legend()
...
plt.plot(x_axis_p, y_axis_mp_p, label='Morris-Pratt Pattern Search O(P)')
plt.legend()
...
plt.plot(x_axis_a, y_axis_mp_a, label='Morris-Pratt Pattern Search O(A)')
plt.legend()
